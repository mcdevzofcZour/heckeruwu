...
Первая команда останавливает сервис и обновляет сертификат. Вторая команда запускает aPuppet с обновленными сертификатами.

Рекомендуется включить эти команды в единый скрипт (apuppet-cert-renewal.sh) и добавить этот скрипт в crontab (запускать каждую неделю в часы наименьшей загрузки):
